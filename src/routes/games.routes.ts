...
const _strategicCtrl = new StrategicGamesController();
const _sessionCtrl = new GameSessionsController();

// Bind all methods so `this` is preserved when passed as Express callbacks
const strategicCtrl = {
  createStrategicSession: _strategicCtrl.createStrategicSession.bind(_strategicCtrl),
  assignStrategicRolesForSession: _strategicCtrl.assignStrategicRolesForSession.bind(_strategicCtrl),
  getMyStrategicRole: _strategicCtrl.getMyStrategicRole.bind(_strategicCtrl),
  acknowledgeMyStrategicRole: _strategicCtrl.acknowledgeMyStrategicRole.bind(_strategicCtrl),
  getStrategicRoleRevealStatus: _strategicCtrl.getStrategicRoleRevealStatus.bind(_strategicCtrl),
  readyMyStrategicRole: _strategicCtrl.readyMyStrategicRole.bind(_strategicCtrl),
  getStrategicRoleReadyStatus: _strategicCtrl.getStrategicRoleReadyStatus.bind(_strategicCtrl),
  getMyStrategicRolePromptState: _strategicCtrl.getMyStrategicRolePromptState.bind(_strategicCtrl),
  advanceMyStrategicRolePrompt: _strategicCtrl.advanceMyStrategicRolePrompt.bind(_strategicCtrl),
  getMyStrategicNotes: _strategicCtrl.getMyStrategicNotes.bind(_strategicCtrl),
  updateMyStrategicNotes: _strategicCtrl.updateMyStrategicNotes.bind(_strategicCtrl),
  getDebriefResults: _strategicCtrl.getDebriefResults.bind(_strategicCtrl),
  startDebrief: _strategicCtrl.startDebrief.bind(_strategicCtrl),
};
const sessionCtrl = {
  getSessionDetails: _sessionCtrl.getSessionDetails.bind(_sessionCtrl),
  getSessionMessages: _sessionCtrl.getSessionMessages.bind(_sessionCtrl),
  exportSessionData: _sessionCtrl.exportSessionData.bind(_sessionCtrl),
...
router.get('/strategic-sessions/:sessionId/debrief-results', authenticateOrGuest, debriefRateLimiter, validate(sessionIdParam, 'params'), strategicCtrl.getDebriefResults);
router.post('/strategic-sessions/:sessionId/start-debrief', authenticateOrGuest, debriefRateLimiter, validate(sessionIdParam, 'params'), strategicCtrl.startDebrief);
