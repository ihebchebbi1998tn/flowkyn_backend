...
export async function handleTwoTruthsAction({
  ...
  const ttRun = ttPrev.then(async () => {
    ...
    console.log('[TwoTruths] Reducer input', {
      actionType: data.actionType,
      sessionId: data.sessionId,
      prevPhase: prevState?.phase || 'null',
      prevSnapshotId: freshLatest?.id || 'null',
      participantId: participant.participantId,
      presenterParticipantId: prevState?.presenterParticipantId || 'null',
    ...
    console.log('[TwoTruths] Reducer output', {
      actionType: data.actionType,
      sessionId: data.sessionId,
      nextPhase: (next as any)?.phase,
      phaseChanged: prevState?.phase !== (next as any)?.phase,
      hasStatements: !!(next as any)?.statements,
      voteCount: Object.keys((next as any)?.votes || {}).length,
    });
    ...
    console.log('[TwoTruths] Broadcasting game:data', {
      actionType: data.actionType,
      sessionId: data.sessionId,
