...
export async function handleCoffeeAction({
  ...
  const run = prevQueue.then(async () => {
    ...
    const prevState = (latestSnapshot?.state as any) || null;

    console.log('[CoffeeRoulette] Reducer input', {
      actionType: normalizedAction,
      sessionId: data.sessionId,
      prevPhase: prevState?.phase || 'null',
      prevSnapshotId: latestSnapshot?.id || 'null',
    ...
    console.log('[CoffeeRoulette] Reducer output', {
      actionType: normalizedAction,
      sessionId: data.sessionId,
      nextPhase: (next as any)?.phase || 'null',
...
export async function handleCoffeeRematchOnLeave(
  ...
  const run = prevQueue.then(async () => {
    try {
      ...
      console.log('[CoffeeRoulette] Rematch on leave: reshuffling', {
        sessionId,
        participantId,
        prevPhase: state?.phase,
        pairsCount: state?.pairs?.length || 0,
      });
      ...
      gamesNs.to(`game:${sessionId}`).emit('game:data', {
        sessionId,
        gameData: next,
        actionType: 'coffee:shuffle_rematch',
        snapshotRevisionId: savedSnapshot?.id || null,
        snapshotCreatedAt: toSnapshotCreatedAt(savedSnapshot?.created_at),
      });
    } catch (err) {
