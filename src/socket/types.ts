...
export interface EventsClientToServer {
  ...
  'chat:message': (data: { eventId: string; participantId: string; message: string }) => void;
  'chat:typing': (data: { eventId: string; participantId: string; isTyping: boolean }) => void;
...
export interface EventsServerToClient {
  ...
  'chat:typing': (data: { participantId: string; userId: string; isTyping: boolean }) => void;
...
export interface GamesClientToServer {
  ...
  'game:action': (data: { sessionId: string; roundId: string; participantId: string; actionType: string; payload: any }) => void;
  'game:round_end': (data: { sessionId: string; roundNumber: number }) => void;
...
export interface GamesServerToClient {
  ...
  'game:round_ended': (data: { sessionId: string; roundNumber: number; timestamp: string }) => void;
  'game:ended': (data: { sessionId: string; results: any; timestamp: string }) => void;
  'game:player_joined': (data: { userId: string; sessionId: string; timestamp: string }) => void;
