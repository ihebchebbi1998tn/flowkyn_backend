...
export function initializeSocket(server: HttpServer) {
  ...
  const isClusterMode = typeof process.env.PM2_INSTANCE_ID !== 'undefined' ||
    ...
    Promise.all([
      ...
      .then(([{ createAdapter }, redis]) => {
        ...
        return Promise.all([
          ...
          console.log('[Socket.IO] ✓ Redis adapter initialized — cross-worker broadcasts enabled');
        });
      })
      .catch((err) => {
        console.error('[Socket.IO] ✗ Failed to initialize Redis adapter:', err?.message || err);
        if (isClusterMode) {
          console.error('[Socket.IO] ⚠ CRITICAL: Running in cluster mode WITHOUT Redis adapter!');
          console.error('[Socket.IO]   game:data broadcasts will NOT reach clients on other workers.');
          console.error('[Socket.IO]   Set REDIS_URL env var or reduce PM2 instances to 1.');
        }
      });
  } else if (isClusterMode) {
    console.error('══════════════════════════════════════════════════════════════');
    console.error('[Socket.IO] ⚠ CRITICAL: Multiple PM2 workers detected but NO Redis adapter!');
    console.error('[Socket.IO]   Socket.IO rooms only work within a single process.');
    console.error('[Socket.IO]   game:data, game:state, chat:message will NOT cross workers.');
    console.error('[Socket.IO]   FIX: Set REDIS_URL=redis://localhost:6379 or use 1 PM2 instance.');
    console.error('══════════════════════════════════════════════════════════════');
  } else {
    console.log('[Socket.IO] Running in single-worker mode (no Redis adapter needed)');
  }
