  ...
  async closeSession(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (req.user) {
        ...
        const allow = await allowParticipantGameControlForEvent(eventId);
        if (!allow) throw new AppError('Only admins can close sessions', 403, 'FORBIDDEN');
        if (req.guest.eventId !== eventId) throw new AppError('Forbidden', 403, 'FORBIDDEN');
      } else {
        throw new AppError('Authentication required', 401, 'AUTH_MISSING_TOKEN');
      }
