...
async function gameTimingSimulation() {
  ...
  // Simulate: state reducer (pure CPU) — realistic single-action reduces
  const reducerStart = performance.now();
  for (let i = 0; i < 1000; i++) {
    // Each iteration simulates ONE action on a small state (realistic)
    const prev = { votes: { p1: 's0', p2: 's1' }, scores: { p1: 100 }, round: 1 };
    const next = { ...prev, votes: { ...prev.votes, [`p${i % 10}`]: 's2' } };
    // Simulate phase transition check
    const _ = next.round > 0 && Object.keys(next.votes).length > 0;
  }
